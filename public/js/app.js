$(document).ready(function() {
    ...
    $('#copyPanBtn').on('click', function() {
        ...
        var text = buildCopyText(pan, window.QIMEN_PAGE_META || {});
 ...
 * 把排盘数据拼成便于分享的纯文本
 */
function buildCopyText(pan, pageMeta) {
    // 按显示宽度补齐（中文/全角占 2 列）
    ...
    if (pageMeta && pageMeta.mode === 'custom') {
        L.push('【自定义奇门排盘】');
        L.push('排盘时间：' + (pageMeta.date || '') + ' ' + (pageMeta.formTime || pageMeta.time || ''));
        L.push('时辰：' + (pageMeta.shichen || '') + '时');
        L.push('时区：' + (pageMeta.timezone || 'Asia/Shanghai'));
        L.push('');
    } else {
        L.push('【实时奇门排盘】');
    }
